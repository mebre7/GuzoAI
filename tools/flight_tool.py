import os
import re
import certifi # this is used to verify the SSL certificate
import airportsdata # this is used to get the airport data
import pycountry # this is used to get the country name from the country code
...
# Default origin when user puts only destination, e.g. "US trip"
DEFAULT_ORIGIN_IATA = os.getenv("DEFAULT_ORIGIN_IATA", "ADD")
...
def clean_input(user_input: str) -> str:
    """
    Clean the user input by removing stop words and extract the location (city or country) from the input."""
    user_input = user_input.lower().strip() # Convert to lowercase and remove leading/trailing spaces
    user_input = re.sub(r"[^a-z0-9\s]", " ", user_input) # Remove non-alphanumeric characters except space
    user_input = re.sub(r"\s+", " ", user_input) # Remove multiple spaces

    stop_words = [
        "flight", "flights", "ticket", "tickets", "trip", "travel", "plan", "complete", "day", "days", "including", "hotel", "hotels", "sightseeing", "under", "budget", "info", "information"
    ]
...
def country_name_to_code(text: str) -> str:
    ...
    # Direct alias match (fastest lookup) (text in alias)
    if text in COUNTRY_ALIASES:
        country_name = text
        return COUNTRY_ALIASES[country_name]

    # Exact pycountry lookup (from text -> pycountry)
    country_name = text
    try:
    ...
    # Substring search in pycountry names or Detect country name inside longer text (from pycountry -> text)
    for country in pycountry.countries:
        py_country = country.name.lower()
        if py_country in text:
            return country.alpha_2
...
def airport_country_matches(airport: dict, country_code: str) -> bool:
    """Check if the airport's country matches the given country code."""
...
def get_best_airport_for_country(country_code: str) -> str:
    """Get the main airport code for a given country code."""
    ...
    for iata, airport in AIRPORTS.items():
        if not iata: # skip if data is None 
            continue 
...
def resolve_location_to_iata(location: str):
    """
    Resolve a location (city or country or airport) to its IATA code.
    Examples:
        "addis ababa" -> "ADD"
    "ethiopia" -> "ET" (country code)
    "new york" -> "JFK" (main airport)
    "jfk" -> "JFK" (already an IATA code)
    "us" -> "US" (country code)
    """
    ...
    location_clean = clean_input(raw_location)
...
if __name__ == "__main__":
    print(search_flights("Plan a 7 days Japan trip from Ethiopia"))
    print("\n" + "=" * 80 + "\n")
